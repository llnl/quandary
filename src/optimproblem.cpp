...
OptimProblem::OptimProblem(const Config& config, TimeStepper* timestepper_, MPI_Comm comm_init_, MPI_Comm comm_optim_, Output* output_, bool quietmode_){
  ...
  gamma_robust = config.getOptimPenaltyRobust();
...
double OptimProblem::evalF(const Vec x) {
  ...
  objective = obj_cost + obj_regul + obj_penal_leakage + obj_penal_dpdm + obj_penal_energy + obj_penal_variation + obj_penal_weightedcost + obj_robust;

  /* Output */
  if (mpirank_world == 0 && !quietmode) {
    std::cout<< "Objective = " << std::scientific<<std::setprecision(14) << obj_cost << " + " << obj_regul << " + " << obj_penal_leakage << " + " << obj_penal_dpdm << " + " << obj_penal_energy << " + " << obj_penal_variation << " + " << obj_penal_weightedcost;
...
void OptimProblem::evalGradF(const Vec x, Vec G){
  ...
  for (int iinit = 0; iinit < ninit_local; iinit++) {
    ...
    if (timestepper->mastereq->decoherence_type != DecoherenceType::NONE) {
      ...
      timestepper->solveAdjointODE(iinit_global, rho_t0_bar, finalstate, obj_weights[iinit] * gamma_penalty_leakage, obj_weights[iinit]*gamma_penalty_weightedcost, obj_weights[iinit]*gamma_penalty_dpdm, obj_weights[iinit]*gamma_penalty_energy);
  ...
  if (timestepper->mastereq->decoherence_type == DecoherenceType::NONE) {
    ...
    for (int iinit = 0; iinit < ninit_local; iinit++) {
      ...
      /* Derivative of time-stepping */
      timestepper->solveAdjointODE(iinit_global, rho_t0_bar, store_finalstates[iinit], obj_weights[iinit] * gamma_penalty_leakage, obj_weights[iinit]*gamma_penalty_weightedcost, obj_weights[iinit]*gamma_penalty_dpdm, obj_weights[iinit]*gamma_penalty_energy);
  ...
  /* Output */
  // if (mpirank_world == 0 && !quietmode) {
  //   std::cout<< "Objective = " << std::scientific<<std::setprecision(14) << obj_cost << " + " << obj_regul << " + " << obj_penal_leakage << " + " << obj_penal_dpdm << " + " << obj_penal_energy << " + " << obj_penal_variation << " + " << obj_penal_weightedcost << " + " << obj_robust <<  std::endl;
  //   std::cout<< "Fidelity = " << fidelity << std::endl;
  // }
}
...
double OptimProblem::computeRobustCost(){
  ...
  for (int n=1; n<=ntime; n++){
    for (int m=n+1; m<=ntime; m++) {
      ...
      // Square and add to objective cost function
      double znm = SQR(znm_re_global[pair_idx]) + SQR(znm_im_global[pair_idx]);
      cost_robust += 2.0*znm;
      pair_idx++;
    }
    // Diagonal shift
...
PetscErrorCode TaoMonitor(Tao tao,void*ptr){
  ...
  if (iter % ctx->output->output_optimization_stride == 0 ||lastIter) {
    ...
    ctx->output->writeOptimFile(iter, f, gnorm, deltax, F_avg, obj_cost, obj_regul, obj_penal_leakage, obj_penal_dpdm, obj_penal_energy, obj_penal_variation, obj_penal_weightedcost, obj_robust);
    // Screen output 
