...
Config::Config(const MPILogger& logger, const toml::table& toml) : logger(logger) {
  try {
    ...
    if (!optimization_table.contains("penalty")) {
      ...
      optim_penalty_robust = ConfigDefaults::OPTIM_PENALTY_ROBUST;
    } else {
      // Parse penalty table
      ...
      optim_penalty_robust = validators::field<double>(*penalty_table, "robust").valueOr(ConfigDefaults::OPTIM_PENALTY_ROBUST);
    }
