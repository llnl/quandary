...
ControlParameterizationSettings parseControlParamSpecsToml(const toml::table& param_table) {
  auto type_str = validators::field<std::string>(param_table, "type").value();
  ...
  if (tstop_opt.has_value()) param.tstop = tstop_opt.value();
  return param;
}
...
ControlInitializationSettings parseControlInitSpecsToml(const toml::table& init_table) {
  ...
  auto filename_opt = validators::getOptional<std::string>((init_table)["filename"]);
...
Config::Config(const toml::table& toml, bool quiet_mode) : Config(quiet_mode) {
  try { 
    ...
    size_t num_osc = nlevels.value().size();

    nessential = validators::scalarOrVectorOr<size_t>(*system_table, "nessential", num_osc, nlevels.value());
    ...
    crosskerr_coupling = std::vector<double>(num_pairs, ConfigDefaults::CROSSKERR_COUPLING);
    dipole_coupling = std::vector<double>(num_pairs, ConfigDefaults::DIPOLE_COUPLING);
    ...
    if (system_table->contains("crosskerr_coupling")) {
      if ((*system_table)["crosskerr_coupling"].is_value()) {
          ...
          crosskerr_coupling = std::vector<double>(num_pairs, single_val);
      } else {
    ...
    if (system_table->contains("dipole_coupling")) {
      if ((*system_table)["dipole_coupling"].is_value()) {
          ...
          dipole_coupling = std::vector<double>(num_pairs, single_val);
      } else {
        auto parseFunc = [](const toml::table& t) { return validators::field<double>(t, "value").value(); };
    ...
    control_zero_boundary_condition = validators::field<bool>(control_table, "zero_boundary_condition").valueOr(ConfigDefaults::CONTROL_ZERO_BOUNDARY_CONDITION);
    ControlParameterizationSettings default_param;
    control_parameterizations = std::vector<ControlParameterizationSettings>(num_osc, default_param);
    ...
    control_initializations = std::vector<ControlInitializationSettings>(num_osc, default_init);
    ...
    carrier_frequencies = std::vector<std::vector<double>>(num_osc, default_carrier_freq);
    if (control_table.contains("carrier_frequency")) {
      ...
      if (carrier_freq_array && !carrier_freq_array->empty() && !carrier_freq_array->front().is_table()) {
        // Direct array format: carrier_frequency = [1.0, 2.0]
          ...
          carrier_frequencies = std::vector<std::vector<double>>(num_osc, values);
      } else {
    ...
    if (optimization_table.contains("target")) {
      auto* target_table = optimization_table["target"].as_table();
      if (!target_table) {
        throw validators::ValidationError("target", "must be a table");
      }
      auto type_str = validators::field<std::string>(*target_table, "type").value();
      auto type_enum = parseEnum(type_str, TARGET_TYPE_MAP);
      if (!type_enum.has_value()) {
        throw validators::ValidationError("target.type", "unknown optim_target type: " + type_str);
      }
    
      OptimTargetSettings target;
      target.type = type_enum.value();
      auto gate_type_str = validators::getOptional<std::string>((*target_table)["gate_type"]);
      if (gate_type_str.has_value()) {
        target.gate_type = parseEnum(gate_type_str.value(), GATE_TYPE_MAP);
        if (!target.gate_type.has_value()) {
          throw validators::ValidationError("target.gate_type", "unknown gate type: " + gate_type_str.value());
        }
      } 
      target.filename = validators::getOptional<std::string>((*target_table)["filename"]);
      target.levels = validators::getOptionalVector<size_t>((*target_table)["levels"]);
      target.gate_rot_freq = validators::scalarOrVectorOr<double>(*target_table, "gate_rot_freq", num_osc, std::vector<double>(num_osc, ConfigDefaults::GATE_ROT_FREQ));
      optim_target = target;
    } else {
    ...
    if (!optimization_table.contains("tikhonov")) {
      ...
      if (!regul_table) {
        throw validators::ValidationError("tikhonov must be a table");
      }
    ...
    if (!optimization_table.contains("penalty")) {
      ...
      if (!penalty_table) {
        throw validators::ValidationError("penalty must be a table");
      }
    ...
    output_observables = std::vector<OutputType>{};
    ...
    throw validators::ValidationError("config", e.what());
  }
...
void Config::printConfig(std::stringstream& log) const {
  ...
  // System parameters
  if (nlevels.has_value()) log << "nlevels = " << printVector(nlevels.value()) << "\n";
  if (nessential.has_value()) log << "nessential = " << printVector(nessential.value()) << "\n";
  if (total_time.has_value()) log << "total_time = " << formatDouble(total_time.value()) << "\n";
  if (timestepper_type != TimeStepperType::PETSCTS) {
    if (ntime.has_value()) log << "ntime = " << ntime.value() << "\n";
    if (dt.has_value()) log << "dt = " << formatDouble(dt.value()) << "\n";
  }
  if (transition_frequency.has_value()) log << "transition_frequency = " << printVector(transition_frequency.value()) << "\n";
  if (selfkerr.has_value()) log << "selfkerr = " << printVector(selfkerr.value()) << "\n";
  if (crosskerr_coupling.has_value()) log << "crosskerr_coupling = " << toStringCoupling(crosskerr_coupling.value(), nlevels.value().size()) << "\n";
  ...
  if (hamiltonian_file_Hc.has_value()) log << "hamiltonian_file_Hc = \"" << hamiltonian_file_Hc.value() << "\"\n";
  ...
  if (optim_target.has_value()) log << "target = " << Config::toString(optim_target.value()) << "\n";
  if (optim_objective.has_value()) log << "objective = \"" << enumToString(optim_objective.value(), OBJECTIVE_TYPE_MAP) << "\"\n";
  if (optim_weights.has_value()) {
    bool uniform_weights = std::adjacent_find(optim_weights.value().begin(), optim_weights.value().end(), std::not_equal_to<double>{}) == optim_weights.value().end();
    if (!uniform_weights) {
      log << "weights = " << printVector(optim_weights.value()) << "\n";
    }
  }
  ...
  if (output_directory.has_value()) log << "directory = \"" << output_directory.value() << "\"\n";
  if (output_observables.has_value()) {
    log << "observables = [";
    for (size_t j = 0; j < output_observables.value().size(); ++j) {
      log << "\"" << enumToString(output_observables.value()[j], OUTPUT_TYPE_MAP) << "\"";
      if (j < output_observables.value().size() - 1) log << ", ";
    }
    log << "]\n";
  }
  if (output_timestep_stride.has_value()) log << "timestep_stride = " << output_timestep_stride.value() << "\n";
  if (output_optimization_stride.has_value()) log << "optimization_stride = " << output_optimization_stride.value() << "\n";
...
void Config::finalize() {
  ...
  if (total_time.value() < 0.0) {
    logger.log("# Warning: total_time not provided or negative. Setting total_time to ntime * dt = " + std::to_string(ntime.value() * dt.value()) + ". It is suggested to provide total_time and remove either ntime or dt from the configuration.\n");
    total_time = ntime.value() * dt.value();
  }
  if (timestepper_type.value() == TimeStepperType::PETSCTS) {
    if (ntime.value() > 0 || dt.value() > 0) {
    ...
    if (ntime.value() > 0 && dt.value() > 0) { // if both are provided, check consistency with total_time.
      double total_time_from_ntime_dt = ntime.value() * dt.value();
      if (std::abs(total_time_from_ntime_dt - total_time.value()) > 1e-6) {
        throw validators::ValidationError("ntime/dt", "ntime * dt = " + std::to_string(total_time_from_ntime_dt) + " is inconsistent with total_time = " + std::to_string(total_time.value()) + ". Provide consistent values for ntime and dt, or provide only one of them and it will be computed from total_time.");
      }
    } else if (ntime.value() > 0) { // if only ntime is provided, compute dt from total_time
      dt = total_time.value() / ntime.value();
    } else if (dt.value() > 0) { // if only dt is provided, compute ntime from total_time
      ntime = static_cast<size_t>(std::round(total_time.value() / dt.value()));
    } else {
  ...
  if ((hamiltonian_file_Hsys.has_value() || hamiltonian_file_Hc.has_value()) && usematfree.value()) {
    logger.log(
  ...
  if (decoherence_type.value() == DecoherenceType::NONE && initial_condition.value().type == InitialConditionType::BASIS) {
    initial_condition.value().type = InitialConditionType::DIAGONAL;
  }

  // For BASIS, ENSEMBLE, and DIAGONAL, or default to all oscillators IDs
  if (initial_condition.value().type == InitialConditionType::BASIS || initial_condition.value().type == InitialConditionType::ENSEMBLE || initial_condition.value().type == InitialConditionType::DIAGONAL) {
    if (!initial_condition.value().subsystem.has_value()) {
      initial_condition.value().subsystem = std::vector<size_t>(nlevels.value().size());
      for (size_t i = 0; i < nlevels.value().size(); i++) {
        initial_condition.value().subsystem->at(i) = i;
      }
    }
  }

  // Compute number of initial conditions
  n_initial_conditions = computeNumInitialConditions(initial_condition.value(), nlevels.value(), nessential.value(), decoherence_type.value());

  // overwrite decay or dephase times with zeros, if the decoherence type is only one of them, or none.
  if (decoherence_type.value() == DecoherenceType::DECAY) {
    std::fill(dephase_time.value().begin(), dephase_time.value().end(), 0);
  } else if (decoherence_type.value() == DecoherenceType::DEPHASE) {
    std::fill(decay_time.value().begin(), decay_time.value().end(), 0);
  } else if (decoherence_type.value() == DecoherenceType::NONE) {
    std::fill(decay_time.value().begin(), decay_time.value().end(), 0);
    std::fill(dephase_time.value().begin(), dephase_time.value().end(), 0);
  }
  ...
  if (optim_weights.value().empty()) {
    optim_weights.value().assign(n_initial_conditions, ConfigDefaults::OPTIM_WEIGHT);
  } else if (optim_weights.value().size() != n_initial_conditions) {
    throw validators::ValidationError("optim_weights vector has length", std::to_string(optim_weights.value().size()) + " but must have length " + std::to_string(n_initial_conditions) + " (number of initial conditions)");
  }
  // Scale the weights so that they sum up to 1
  double scaleweights = 0.0;
  for (size_t i = 0; i < optim_weights.value().size(); i++) scaleweights += optim_weights.value()[i];
  if (scaleweights == 0.0) {
    throw validators::ValidationError("optim_weights sum to zero; at least one weight must be positive");
  }
  for (size_t i = 0; i < optim_weights.value().size(); i++) optim_weights.value()[i] = optim_weights.value()[i] / scaleweights;

  // Set weightedcost width to zero if weightedcost penalty is zero
  if (optim_penalty_weightedcost.value() == 0.0) {
  ...
  for (size_t i = 0; i < control_parameterizations.value().size(); i++) {
    if (control_parameterizations.value()[i].type != ControlType::BSPLINE0) {
  ...
  if (optim_target.value().type == TargetType::GATE) {
    if (optim_target.value().filename.has_value()) {
      optim_target.value().gate_type = GateType::FILE;
...
void Config::validate() const {
  ...
  if (nessential.value().size() != nlevels.value().size()) {
    throw validators::ValidationError("nessential size must match nlevels size");
  }
  for (size_t i = 0; i < nlevels.value().size(); i++) {
    if (nessential.value()[i] > nlevels.value()[i]) {
      throw validators::ValidationError("nessential[" + std::to_string(i) + "] = " + std::to_string(nessential.value()[i]) + " cannot exceed nlevels[" + std::to_string(i) + "] = " + std::to_string(nlevels.value()[i]));
    }
  }
  ...
  for (size_t i = 0; i < control_initializations.value().size(); i++) {
    const auto& init = control_initializations.value()[i];
  ...
  if (optim_target.value().type == TargetType::GATE) {
    if (!optim_target.value().gate_type.has_value() && !optim_target.value().filename.has_value()) {
      throw validators::ValidationError("optimization target of type GATE requires 'gate_type' or 'filename'");
    }
    if (optim_target.value().gate_rot_freq.has_value() && optim_target.value().gate_rot_freq->size() != nlevels.value().size()) {
      throw validators::ValidationError("optimization target gate_rot_freq size (" + std::to_string(optim_target.value().gate_rot_freq->size()) + ") must match number of oscillators (" + std::to_string(nlevels.value().size()) + ")");
    }
  } else if (optim_target.value().type == TargetType::STATE) {
    if (!optim_target.value().levels.has_value() && !optim_target.value().filename.has_value()) {
    ...
    if (optim_target.value().levels.has_value()) {
      if (optim_target.value().levels->size() != nlevels.value().size()) {
        throw validators::ValidationError("optimization target levels size (" + std::to_string(optim_target.value().levels->size()) + ") must match number of oscillators (" + std::to_string(nlevels.value().size()) + ")");
      }
      for (size_t i = 0; i < optim_target.value().levels->size(); i++) {
        if (optim_target.value().levels->at(i) >= nlevels.value()[i]) {
          throw validators::ValidationError("optimization target levels[" + std::to_string(i) + "] = " + std::to_string(optim_target.value().levels->at(i)) + " exceeds number of modeled levels (" + std::to_string(nlevels.value()[i]) + ")");
        }
      }
    }
  }
  ...
  if (decoherence_type.value() == DecoherenceType::NONE) {
    if (initial_condition.value().type == InitialConditionType::ENSEMBLE ||
        initial_condition.value().type == InitialConditionType::THREESTATES ||
        initial_condition.value().type == InitialConditionType::NPLUSONE) {
  ...
  if (initial_condition.value().type == InitialConditionType::FROMFILE) {
    if (!initial_condition.value().filename.has_value()) {
      throw validators::ValidationError("initialcondition of type FROMFILE must have a filename");
    }
  }
  if (initial_condition.value().type == InitialConditionType::PRODUCT_STATE) {
    if (!initial_condition.value().levels.has_value()) {
      throw validators::ValidationError("initialcondition of type PRODUCT_STATE must have 'levels'");
    }
    if (initial_condition.value().levels->size() != nlevels.value().size()) {
      throw validators::ValidationError("initialcondition of type PRODUCT_STATE must have exactly " + std::to_string(nlevels.value().size()) + " parameters, got " + std::to_string(initial_condition.value().levels->size()));
    }
    for (size_t k = 0; k < initial_condition.value().levels->size(); k++) {
      if (initial_condition.value().levels->at(k) >= nlevels.value()[k]) {
        throw validators::ValidationError("ERROR in config setting. The requested product state initialization " + std::to_string(initial_condition.value().levels->at(k)) + " exceeds the number of allowed levels for that oscillator (" + std::to_string(nlevels.value()[k]) + ").\n");
      }
    }
  }
  if (initial_condition.value().type == InitialConditionType::BASIS ||
      initial_condition.value().type == InitialConditionType::DIAGONAL ||
      initial_condition.value().type == InitialConditionType::ENSEMBLE) {
    if (!initial_condition.value().subsystem.has_value()) {
      throw validators::ValidationError("initialcondition of type BASIS, DIAGONAL, or ENSEMBLE must have 'subsystem'");
    }
    if (initial_condition.value().subsystem->back() >= nlevels.value().size()) {
      throw validators::ValidationError("Last element in initialcondition params exceeds number of oscillators");
    }
    for (size_t i = 1; i < initial_condition.value().subsystem->size() - 1; i++) {
      if (initial_condition.value().subsystem->at(i) + 1 != initial_condition.value().subsystem->at(i + 1)) {
  ...
  if (timestepper_type.value() == TimeStepperType::PETSCTS) {
    ...
    if (runtype.value() != RunType::SIMULATION && optim_penalty_energy.value() > 1e-13 && optim_penalty_leakage.value() > 1e-13) {
