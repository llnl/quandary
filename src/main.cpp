...
int main(int argc,char **argv)
{
  ...
  double gamma_robust = config.getOptimPenaltyRobust();
  if (gamma_robust > 0.0) storeFWD = true;
  // Stop if lindblad solver and robust optimization, as its not implemented.
  if (gamma_robust > 0.0 && mastereq->decoherence_type != DecoherenceType::NONE) {
  ...
  switch (timesteppertype) {
    case TimeStepperType::IMR:
      mytimestepper = new ImplMidpoint(mastereq, ntime, total_time, linsolvetype, linsolve_maxiter, output, storeFWD, ninit/mpisize_init, gamma_robust > 0.0);
      break;
    case TimeStepperType::IMR4:
      mytimestepper = new CompositionalImplMidpoint(4, mastereq, ntime, total_time, linsolvetype, linsolve_maxiter, output, storeFWD, ninit/mpisize_init, gamma_robust > 0.0);
      break;
    case TimeStepperType::IMR8:
      mytimestepper = new CompositionalImplMidpoint(8, mastereq, ntime, total_time, linsolvetype, linsolve_maxiter, output, storeFWD, ninit/mpisize_init, gamma_robust > 0.0);
      break;
    case TimeStepperType::EE:
      mytimestepper = new ExplEuler(mastereq, ntime, total_time, output, storeFWD, ninit/mpisize_init, gamma_robust > 0.0);
      break;
    default:
  ...
  /* Output */
  if (runtype != RunType::OPTIMIZATION) {
    optimctx->output->writeOptimFile(0, optimctx->getObjective(), gnorm, 0.0, optimctx->getFidelity(), optimctx->getCostT(), optimctx->getRegul(), optimctx->getPenaltyLeakage(), optimctx->getPenaltyDpDm(), optimctx->getPenaltyEnergy(), optimctx->getPenaltyVariation(), optimctx->getPenaltyWeightedCost(), optimctx->getObjRobust());
  }
