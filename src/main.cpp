...
int main(int argc,char **argv)
{
  ...
  // If runtype is gradient or optimization, store the forward trajectory. Exceptions: If Schroedinger solver and non-Petsc Timestepper, then states are recomputed during the backward solves, so no need to store them. 
  bool storeFWD = false;
  RunType runtype = config.getRuntype();
  if (runtype == RunType::GRADIENT || runtype == RunType::OPTIMIZATION) {
    storeFWD = true;  
    if (config.getDecoherenceType() == DecoherenceType::NONE && config.getTimestepperType() != TimeStepperType::PETSCTS) 
      storeFWD=false;
  }
