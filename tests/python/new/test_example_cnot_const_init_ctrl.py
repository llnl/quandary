...
def test_example_cnot_const_init_ctrl(tmp_path, request):
    """Test CNOT gate optimization with constant (non-random) initial controls using new Python interface."""
    ...
    # Match old initialization amplitude: initctrl_MHz=100.0, randomize_init_ctrl=False
    num_carrier_freqs = len(setup.carrier_frequencies[0])
    init_amplitude = 100.0 / 1000.0 / np.sqrt(2) / num_carrier_freqs
